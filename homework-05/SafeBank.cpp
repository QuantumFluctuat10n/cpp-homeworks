...
void cashierWork(SafeBankAccount &bankAccount, SafeBankStatistics &statist)
{
...
int main()
{
    ...
    SafeBankStatistics statist;
    std::vector<std::thread> threads(5);
    for (int i = 0; i < 5; i++)
    {
        threads.emplace_back(cashierWork, std::ref(safeAccount), std::ref(statist));
    }
    ...
    std::cout << "Final balance: " << safeAccount.get_balance() << std::endl;
}